...
struct NativePanelView: View {
    ...
    private var content: some View {
        if loading && series.isEmpty && rows.isEmpty {
            ...
            Text(error).font(.caption).foregroundStyle(.secondary).frame(minHeight: 80)
        } else {
            switch panel.kind {
...
// MARK: - Timeseries (taller on iPad)
...
struct TimeseriesChart: View {
    ...
    var body: some View {
        if series.isEmpty {
            Text("No data").font(.caption).foregroundStyle(.secondary).frame(minHeight: 80)
        } else {
            Chart {
                ForEach(series) { s in
            ...
            .chartLegend(.hidden)
            .frame(height: 200)  // taller than iPhone's 160 pt
